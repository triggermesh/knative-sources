...
const (
	...
	// ReasonAdapterDelete indicates that an adapter object was successfully deleted.
	ReasonAdapterDelete = "DeleteAdapter"
)
...
func newKServiceCreated(namespace, name string) pkgreconciler.Event {
	return pkgreconciler.NewEvent(corev1.EventTypeNormal, ReasonAdapterCreate, "Created knative service: \"%s/%s\"", namespace, name)
}
...
func newKServiceUpdated(namespace, name string) pkgreconciler.Event {
	return pkgreconciler.NewEvent(corev1.EventTypeNormal, ReasonAdapterUpdate, "Updated knative service: \"%s/%s\"", namespace, name)
}
...
func newKServiceDeleted(namespace, name string) pkgreconciler.Event {
	return pkgreconciler.NewEvent(corev1.EventTypeWarning, ReasonAdapterDelete, "Deleted knative service: \"%s/%s\"", namespace, name)
}
...
func (r *kServiceReconciler) ReconcileKService(ctx context.Context, owner kmeta.OwnerRefable, expected *servingv1.Service) (*servingv1.Service, pkgreconciler.Event) {
	ksvcs, err := r.findOwned(ctx, owner)

	switch {
	case apierrors.IsNotFound(err):

		ksvc, err := r.servingClientSet.ServingV1().Services(expected.Namespace).Create(expected)
		if err != nil {
			return nil, err
		}
	...
	case err != nil:
		return nil, fmt.Errorf("error getting receive knative service adapter %q: %v", expected.Name, err)
	...
	// Owned Knative service with an incorrect name is not expected.
	// If found, delete and let the controller create a new one.
	if ksvc.Name != expected.Name {
		logging.FromContext(ctx).Warnf("Deleting KService %s/%s owned by %s because its name differs "+
			"from expected (%s)", ksvc.Namespace, ksvc.Name, owner.GetObjectMeta().GetName(), expected.Name)

		err = r.servingClientSet.ServingV1().Services(ksvc.Namespace).Delete(ksvc.Name, &metav1.DeleteOptions{})
		if err != nil {
			return nil, err
		}
	...
	if Semantic.DeepEqual(expected, ksvc) {
		return ksvc, nil
	}
	...
	ksvc, err = r.servingClientSet.ServingV1().Services(ksvc.Namespace).Update(expected)
	if err != nil {
		return nil, err
	}
...
// findOwned returns running KServices owned by the passed object.
func (r *kServiceReconciler) findOwned(ctx context.Context, owner kmeta.OwnerRefable) ([]*servingv1.Service, error) {
	klist, err := r.servingLister.Services(owner.GetObjectMeta().GetNamespace()).List(labels.Everything())
	if err != nil {
		return nil, err
	}

	ksvcs := []*servingv1.Service{}
	for _, ksvc := range klist {
		if metav1.IsControlledBy(ksvc, owner.GetObjectMeta()) && ksvc.DeletionTimestamp.IsZero() {
