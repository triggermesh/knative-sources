...
// ZendeskEventAPIHandler listen for Zendesk API Events
type ZendeskEventAPIHandler interface {
	Start(stopCh <-chan struct{}) error
}

type zendeskEventAPIHandler struct {
	port  int
	token string
	appID string
...
// NewZendeskEventAPIHandler creates the default implementation of the Zendesk API Events handler
func NewZendeskEventAPIHandler(ceClient cloudevents.Client, port int, token, appID string, logger *zap.SugaredLogger) ZendeskEventAPIHandler {
	return &zendeskEventAPIHandler{
		port:  port,
		token: token,
		appID: appID,
...
func (h *zendeskEventAPIHandler) Start(stopCh <-chan struct{}) error {
...
func (h *zendeskEventAPIHandler) handleAll(w http.ResponseWriter, r *http.Request) {
	if r.Body == nil {
	...
	// to return 2xx withing 3 seconds to Zendesk API, otherwise
	...
	// - `event_callback`, See: https://api.zendesk.com/events-api#receiving_events
	// - `event_callback`, See: https://api.zendesk.com/events-api#subscriptions
	switch event.Type {
	case "event_callback":
...
func (h *zendeskEventAPIHandler) gracefulShutdown(stopCh <-chan struct{}, done chan<- bool) {
	<-stopCh
...
func (h *zendeskEventAPIHandler) handleError(err error, w http.ResponseWriter) {
	h.logger.Error("An error ocurred", zap.Error(err))
	http.Error(w, err.Error(), http.StatusInternalServerError)
}

func (h *zendeskEventAPIHandler) handleChallenge(body []byte, w http.ResponseWriter) {
	...
	cr := &ZendeskChallengeResponse{Challenge: c.Challenge}
...
func (h *zendeskEventAPIHandler) handleCallback(wrapper *ZendeskEventWrapper, w http.ResponseWriter) {
