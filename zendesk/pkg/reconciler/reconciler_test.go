...
import (
	...
	fakeinjectionclient "github.com/triggermesh/knative-sources/zendesk/pkg/client/generated/injection/client/fake"
	reconcilerv1alpha1 "github.com/triggermesh/knative-sources/zendesk/pkg/client/generated/injection/reconciler/sources/v1alpha1/zendesksource"
	st "github.com/triggermesh/knative-sources/zendesk/pkg/reconciler/testing"
)
	...
	testCases := rt.TableTest{
...

		{
			...
			WantStatusUpdates: []clientgotesting.UpdateActionImpl{{
				Object: makeZendeskSource(
					sourceWithAdapter(makeZendeskAdapter()),
					sourceWithSink,
...
func reconcilerCtor(ctx context.Context, ls *st.Listers) controller.Reconciler {
	...
	return reconcilerv1alpha1.NewReconciler(ctx, logger, fakeinjectionclient.Get(ctx), ls.GetZendeskSourceLister(), controller.GetEventRecorder(ctx), r)
}

// Zendesk Source

type sourceOpt func(*v1alpha1.ZendeskSource)

func makeZendeskSource(opts ...sourceOpt) *v1alpha1.ZendeskSource {
	addr := newAdressable()
	addrGVK := addr.GetGroupVersionKind()

	o := &v1alpha1.ZendeskSource{
...
func sourceWithAdapter(s *servingv1.Service) sourceOpt {
	return func(ss *v1alpha1.ZendeskSource) {
		ss.Status.PropagateAvailability(s)
	}
}

func sourceWithSink(s *v1alpha1.ZendeskSource) {
	s.Status.MarkSink(tSinkURI)
}

func sourceWithCloudEventsAttr(s *v1alpha1.ZendeskSource) {
	s.Status.CloudEventAttributes = []duckv1.CloudEventAttributes{{Type: v1alpha1.ZendeskSourceEventType}}
}

// deleted marks the source as deleted.
func sourceWithDeletionTimestamp(src *v1alpha1.ZendeskSource) {
	t := metav1.Unix(0, 0)
...
// makeZendeskAdapter returns a test Service object with pre-filled attributes.
func makeZendeskAdapter(opts ...adapterOpt) *servingv1.Service {
